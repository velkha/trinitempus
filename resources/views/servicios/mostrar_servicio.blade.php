...
@section('zonaExtra')
    <div class="row" >
        ...
        <div class="col-1"></div>
        <div class="col-2">
            <a href="{{url('/contenido_servicio/'.$data->id)}}" class="standard">Contenido multimedia</a>
        </div>
        <div class="col-2"></div>
    </div>
    <div class="row">
        <div class="col-12">
            ...
            @foreach(\App\Http\Controllers\ComentarioController::getComentariosFromServicio($data->id) as $comentario)
                @if($comentario->privado==0||$comentario->id_usuario==Auth::user()->id||$data->getOwner->id==Auth::user()->id)
